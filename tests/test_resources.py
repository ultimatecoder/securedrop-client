...
def test_path(mocker):
    ...
    """
    mock_res = mocker.patch('securedrop_client.resources.resource_filename',
                            return_value='bar')
    assert securedrop_client.resources.path('foo') == 'bar'
    mock_res.assert_called_once_with(securedrop_client.resources.__name__,
                                     'images/foo')
...
def test_load_css(mocker):
    ...
    """
    mock_rs = mocker.patch('securedrop_client.resources.resource_string',
                           return_value=b'foo')
    assert 'foo' == securedrop_client.resources.load_css('foo')
    mock_rs.assert_called_once_with(securedrop_client.resources.__name__,
                                    'css/foo')
